...
func (s *thingsService) SaveThing(
	...
	if err != nil {
		logger.Error("Save Thing error:", err)
		return err
	}
	return nil
}
...
func (s *thingsService) GetThing(ctx context.Context, id string) (*models.Thing, error) {
	...
	if err != nil {
		logger.Error("Get Thing error:", err)
		return nil, err
	}
	return thing, nil
}
...
func (s *thingsService) DeleteThing(ctx context.Context, id string) (string, error) {
	...
	if err != nil {
		logger.Error("Could Delete and ThingId token in cache. err: ", err)
	}
	resId, err := s.repository.Delete(ctx, id)
	if err != nil {
		logger.Error("Delete Thing error:", err)
		return "", err
	}
	return resId, nil
}
...
func (s *thingsService) UpdateThing(
	...
	if err != nil {
		logger.Error("Update Thing error:", err)
		return err
	}
	return nil
}
...
func (s *thingsService) GetThingToken(
	...
	if err != nil {
		logger.Error("Get thing token error: ", err)
		return "", err
	}
	err = s.cache.SetTokenWithThingId(jwt, thingId)
	if err != nil {
		logger.Error("Could not set token in cache. err: ", err)
	}
	return jwt, nil
}
