...
import (
	...
	mock "github.com/pseudoincorrect/bariot/things/mock/service"
...
func TestThingGetEndpoint(t *testing.T) {
	...
	testData := []struct {
		...
		},
		{
			...
			thing:   nil,
			want:    errors.ErrThingNotFound.Error() + "\n",
			throw:   "",
		},
		{
...
func TestThingPostEndpoint(t *testing.T) {
	...
	bodyCorrect := []byte(`{"Name":"SmartDispenser1","Key":"1234"}`)
	mockThing := CreateMockThing()
	testData := []struct {
		name   string
		userId string
